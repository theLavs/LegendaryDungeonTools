...
LDT.scaleMultiplier[dungeonIndex] = 0.6

LDT.dungeonTotalCount[dungeonIndex] = {normal=261,teeming=313,teemingEnabled=true}
...
function LDT:ToggleFreeholdSelector(show)
    db = LDT:GetDB()
    if not selectorGroup then
        ...
        selectorGroup.frame:SetBackdropColor(unpack(LDT.BackdropColor))
        ...
        selectorGroup.frame.bg:SetColorTexture(unpack(LDT.BackdropColor))
        selectorGroup:SetWidth(120)
        selectorGroup:SetHeight(90)
        selectorGroup.frame:SetPoint("TOPRIGHT", LDT.main_frame,"TOPRIGHT",0,0)

    end
    LDT:UpdateFreeholdSelector(LDT:GetCurrentPreset().week)
    if show then
        selectorGroup.frame:Show()
        LDT:DungeonEnemies_UpdateFreeholdCrew(LDT:GetCurrentPreset().freeholdCrew)
...
function LDT:UpdateFreeholdSelector(week)
    if not selectorGroup then return end
    week = week%3
    if week == 0 then week = 3 end
    selectorGroup:ReleaseChildren()
    LDT:GetCurrentPreset().freeholdCrew = (LDT:GetCurrentPreset().freeholdCrew and week) or nil
    ...
    check:SetCallback("OnValueChanged",function(widget,callbackName,value)
        LDT:GetCurrentPreset().freeholdCrew = (value and week) or nil
        if LDT.liveSessionActive and LDT:GetCurrentPreset().uid == LDT.livePresetUID then
            LDT:LiveSession_SendFreeholdSelector(value,week)
        end
        LDT:DungeonEnemies_UpdateFreeholdCrew(LDT:GetCurrentPreset().freeholdCrew)
        LDT:ReloadPullButtons()
        LDT:UpdateProgressbar()
    end)
    check:SetValue(LDT:GetCurrentPreset().freeholdCrew)
    LDT:DungeonEnemies_UpdateFreeholdCrew(LDT:GetCurrentPreset().freeholdCrew)
end

LDT.mapPOIs[dungeonIndex] = {
    [1] = {
        [1] = {
            ["y"] = -200.49488376925;
            ["x"] = 591.8001100269;
