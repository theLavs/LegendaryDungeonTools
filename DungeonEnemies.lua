...
function LDT:GetDungeonEnemyBlips()
    return blips
end
...
function LDT:DoFramesOverlap(frameA, frameB, offset)
        ...
        if not self.selected then
            ...
            for poiFrame,_ in pairs(active) do
                if poiFrame.spireIndex and poiFrame.npcId == self.data.id then
                    ...
                    self.animatedLine = LDT:ShowAnimatedLine(LDT.main_frame.mapPanelFrame,self.spireFrame,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
                    self.spireFrame.animatedLine = self.animatedLine
                    break
                end
            end
            local connectedDoor = LDT:FindConnectedDoor(self.data.id)
            if connectedDoor then
                self.animatedLine = LDT:ShowAnimatedLine(LDT.main_frame.mapPanelFrame,connectedDoor,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
            end
        end
    end
    if not db.devMode then
        self.fontstring_Text1:SetText(LDT:IsCurrentPresetTeeming() and self.data.teemingCount or self.data.count)
        ...
        for _,blip in pairs(blips) do
            if blip.clone.g == self.clone.g then
                blip.fontstring_Text1:SetText(LDT:IsCurrentPresetTeeming() and blip.data.teemingCount or blip.data.count)
...
function LDTDungeonEnemyMixin:OnLeave()
    self:updateSizes(1)
    self:SetFrameLevel(self:GetFrameLevel()-5)
    if db.devMode then
    ...
    LDT:DisplayBlipTooltip(self,false)
    if self.data.corrupted then
        ...
        if not self.selected then
            LDT:HideAnimatedLine(self.animatedLine)
        end
    end
    if not db.devMode then
        if not self.clone.g then
...
local function setUpMouseHandlers(self)
    ...
    self:SetScript("OnDragStart", function()
        local x,y,scale
        preset = LDT:GetCurrentPreset()
        tempPulls = LDT:DeepCopy(preset.value.pulls)
        local targetPull
        local _, _, _, blipX, blipY = self:GetPoint()
        self:SetScript("OnUpdate",function()
            local nx,ny = LDT:GetCursorPosition()
            if x~=nx or y~=ny then
                x,y = nx,ny
                --find closest pull and measure distance
                local pullIdx,centerX,centerY = LDT:FindClosestPull(x,y)
                ...
                if not isClose then
                    targetPull = nil
                    LDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self, false, IsControlKeyDown(),tempPulls,nil,true)
                    LDT:DungeonEnemies_UpdateSelected(LDT:GetCurrentPull(),tempPulls)
                elseif pullIdx~=targetPull then
                    targetPull = pullIdx
                    LDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self, true, IsControlKeyDown(),tempPulls,pullIdx,true)
                    LDT:DungeonEnemies_UpdateSelected(LDT:GetCurrentPull(),tempPulls)
                end
            end
        end)
...
local function setUpMouseHandlersAwakened(self,clone,scale,riftOffsets)
    ...
    self:SetScript("OnMouseDown",function(self, button)
        if button == "LeftButton" then
            riftOffsets = LDT:GetRiftOffsets()
    ...
    self:SetScript("OnDragStart", function()
        ...
        self:SetScript("OnUpdate",function()
            ...
            if x ~= self.adjustedX or y~= self.adjustedY then
                local sizex,sizey = LDT:GetDefaultMapPanelSize()
                x = x<=0 and 0 or x>=sizex and sizex or x
                y = y>=0 and 0 or y<=(-1)*sizey and (-1)*sizey or y
                self.adjustedX = x
                self.adjustedY = y
                local connectedDoor = LDT:FindConnectedDoor(self.data.id)
                self.animatedLine = LDT:ShowAnimatedLine(LDT.main_frame.mapPanelFrame,connectedDoor or self.spireFrame ,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
                riftOffsets[self.data.id] = riftOffsets[self.data.id] or {}
                riftOffsets[self.data.id].x = x
                riftOffsets[self.data.id].y = y
                self:ClearAllPoints()
                self:SetPoint("CENTER", LDT.main_frame.mapPanelTile1,"TOPLEFT",x*scale,y*scale)
            end
        end)
    end)
    self:SetScript("OnDragStop", function()
        ...
        self:SetScript("OnUpdate",nil)
        self:ClearAllPoints()
        self:SetPoint("CENTER", LDT.main_frame.mapPanelTile1,"TOPLEFT",self.adjustedX*scale,self.adjustedY*scale)
        --dragged ontop of door
        --find doors,check overlap,break,swap sublevel,change poi sublevel
        local _,active = LDT.poi_framePools:GetPool("MapLinkPinTemplate"):EnumerateActive()
        for poiFrame,_ in pairs(active) do
            if LDT:DoFramesOverlap(self,poiFrame,-10) then
                ...
                if shouldAdd then tinsert(c,{ connectionIndex=poiFrame.poi.connectionIndex, source= LDT:GetCurrentSubLevel()+0, target=poiFrame.poi.target}) end
        ...
        if LDT.liveSessionActive and LDT:GetCurrentPreset().uid == LDT.livePresetUID then
            LDT:LiveSession_SendCorruptedPositions(preset.value.riftOffsets)
        end
    end)
end

function LDTDungeonEnemyMixin:OnClick(button, down)

    if button == "LeftButton" then
        if IsShiftKeyDown() then
            local newPullIdx = LDT:GetCurrentPull() + 1
            LDT:PresetsAddPull(newPullIdx)
            LDT:GetCurrentPreset().value.selection = { newPullIdx}
            LDT:ReloadPullButtons()
            LDT:SetSelectionToPull(newPullIdx)
            local _
            LDT:ColorAllPulls(_, newPullIdx)
            --if LDT:GetPullsNum() == 2 then LDT:SetAutomaticColor(1) end
        end
        LDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(self,not self.selected,IsControlKeyDown())
        LDT:DungeonEnemies_UpdateSelected(LDT:GetCurrentPull())
        ...
        if self.data.corrupted then
            ...
            self.animatedLine = LDT:ShowAnimatedLine(LDT.main_frame.mapPanelFrame,connectedFrame,self,nil,nil,nil,nil,nil,self.selected,self.animatedLine)
    ...
    elseif button == "RightButton" then
        if db.devMode then
            if IsAltKeyDown() then
                tremove(LDT.dungeonEnemies[db.currentDungeonIdx][self.enemyIdx].clones,self.cloneIdx)
                self:Hide()
            else
...
function LDT:GetPatrolBlips()
    return patrolPoints
end

function LDTDungeonEnemyMixin:DisplayPatrol(shown)
    local scale = LDT:GetScale()
    ...
    if self.clone.patrol then
        ...
        for patrolIdx,waypoint in ipairs(self.clone.patrol) do
            patrolPoints[patrolIdx] = patrolPoints[patrolIdx] or LDT.main_frame.mapPanelFrame:CreateTexture("LDTDungeonPatrolPoint"..patrolIdx,"BACKGROUND")
            ...
            patrolPoints[patrolIdx]:SetPoint("CENTER", LDT.main_frame.mapPanelTile1,"TOPLEFT",waypoint.x*scale,waypoint.y*scale)
            patrolPoints[patrolIdx].x = waypoint.x
            patrolPoints[patrolIdx].y = waypoint.y
            patrolPoints[patrolIdx]:Show()

            patrolLines[patrolIdx] = patrolLines[patrolIdx] or LDT.main_frame.mapPanelFrame:CreateTexture("LDTDungeonPatrolLine"..patrolIdx,"BACKGROUND")
            patrolLines[patrolIdx]:SetDrawLayer("OVERLAY", 1)
            patrolLines[patrolIdx]:SetTexture("Interface\\AddOns\\legendarydungeontools\\Textures\\Square_White")
            ...
            if oldWaypointBlip then
                ...
                DrawLine(patrolLines[patrolIdx], LDT.main_frame.mapPanelTile1, startX, startY, endX, endY, 1*scale, 1,"TOPLEFT")
                patrolLines[patrolIdx]:Show()
            else
        ...
        if firstWaypointBlip and oldWaypointBlip then
            ...
            local endPoint, endRelativeTo, endRelativePoint, endX, endY = oldWaypointBlip:GetPoint()
            DrawLine(patrolLines[1], LDT.main_frame.mapPanelTile1, startX, startY, endX, endY, 1*scale, 1,"TOPLEFT")
            patrolLines[1]:Show()
        end
...
local ranOnce
function LDT:DisplayBlipTooltip(blip, shown)
    if not ranOnce then
        --fix elvui skinning
        LDT.tooltip:ClearAllPoints()
        LDT.tooltip:SetPoint("TOPLEFT",UIParent,"BOTTOMRIGHT")
        LDT.tooltip:SetPoint("BOTTOMRIGHT",UIParent,"BOTTOMRIGHT")
        LDT.tooltip:Show()
        LDT.tooltip:Hide()
        ranOnce = true
    end
    ...
    local health = LDT:CalculateEnemyHealth(boss,data.health,db.currentDifficulty,data.ignoreFortified)
    ...
    local text = data.name.." "..occurence..group.."\n"..string.format(L["Level %d %s"],data.level,data.creatureType).."\n".. string.format(L["%s HP"],LDT:FormatEnemyHealth(health)).."\n"
    local count = LDT:IsCurrentPresetTeeming() and data.teemingCount or data.count
    text = text ..L["Forces"]..": ".. LDT:FormatEnemyForces(count)
    local reapingText
    if blip.data.reaping and db.MDI.enabled and preset.mdi.beguiling == 13 then
        local reapingIcon = CreateTextureMarkup(LDT.reapingStatic[tostring(blip.data.reaping)].iconTexture, 32, 32, 16, 16, 0, 1, 0, 1,0,0) or ""
        reapingText = L["Reaping"]..": "..reapingIcon.." ".. LDT.reapingStatic[tostring(blip.data.reaping)].name .. "\n"
    end
    ...
    if db.tooltipInCorner then
        tooltip:SetPoint("BOTTOMRIGHT", LDT.main_frame,"BOTTOMRIGHT",0,0)
        tooltip:SetPoint("TOPLEFT", LDT.main_frame,"BOTTOMRIGHT",-tooltip.mySizes.x,tooltip.mySizes.y)
    else
        ...
        local mainFrameRight = LDT.main_frame:GetRight()
...
function LDT:GetCurrentDevmodeBlip()
    for blipIdx,blip in pairs(blips) do
        if blip.devSelected then
            return blip
        end
...
local function blipDevModeSetup(blip)
    ...
    blip:SetScript("OnMouseDown",function()
        ...
        x = x*(1/scale)
        y = y*(1/scale)
        local nx = LDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].x
        local ny = LDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].y
        xOffset = x-nx
        yOffset = y-ny
    end)
    ...
    blip:SetScript("OnDragStop", function()
        ...
        blip:ClearAllPoints()
        blip:SetPoint("CENTER", LDT.main_frame.mapPanelTile1,"TOPLEFT",x*scale,y*scale)
        LDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].x = x
        LDT.dungeonEnemies[db.currentDungeonIdx][blip.enemyIdx].clones[blip.cloneIdx].y = y
    end)
    local groupColors = {
        [1] = {1,0,0,1},
        [2] = {0,1,0,1},
    ...
    blip:SetScript("OnMouseWheel", function(self, delta)
        ...
        if not blip.clone.g then
            local maxGroup = 0
            for _,data in pairs(LDT.dungeonEnemies[db.currentDungeonIdx]) do
...
function LDTDungeonEnemyMixin:SetUp(data,clone)
    local scale = LDT:GetScale()
    self:ClearAllPoints()
    self:SetPoint("CENTER", LDT.main_frame.mapPanelTile1,"TOPLEFT",clone.x*scale,clone.y*scale)
    self.normalScale = data.scale*(data.isBoss and 1.7 or 1)*(LDT.scaleMultiplier[db.currentDungeonIdx] or 1)*scale
    ...
    if not data.corrupted then
        for k,v in pairs(blips) do
            ...
            if ((clone.x-v.clone.x)^2+(clone.y-v.clone.y)^2<81) and LDT:DoFramesOverlap(self, v,5) then raise = max(raise,v:GetFrameLevel()+1) end
        end
    end
    ...
    if data.corrupted then
        ...
        local riftOffsets = LDT:GetRiftOffsets()
...
function LDT:DungeonEnemies_IsAnyBlipMoving()
    local isAnyMoving
    for blipIdx,blip in pairs(blips) do
...
function LDT:DungeonEnemies_HideAllBlips()
    LDT.dungeonEnemies_framePools:ReleaseAll()
end

function LDT:DungeonEnemies_UpdateEnemies()
    LDT.dungeonEnemies_framePools:ReleaseAll()
    twipe(blips)
    if not db then db = LDT:GetDB() end
    local enemies = LDT.dungeonEnemies[db.currentDungeonIdx]
        ...
        for cloneIdx,clone in pairs(data["clones"]) do
            ...
            if clone.sublevel == currentSublevel or (not clone.sublevel) then
                ...
                if not (data.corrupted and riftOffsets and riftOffsets[data.id] and riftOffsets[data.id].sublevel) then
                    local blip = LDT.dungeonEnemies_framePools:Acquire("LDTDungeonEnemyTemplate")
...
function LDT:DungeonEnemies_CreateFramePools()
    db = self:GetDB()
    LDT.dungeonEnemies_framePools = LDT.dungeonEnemies_framePools or CreateFramePoolCollection()
    LDT.dungeonEnemies_framePools:CreatePool("Button", LDT.main_frame.mapPanelFrame, "LDTDungeonEnemyTemplate");
end

function LDT:FindPullOfBlip(blip)
...
function LDT:GetBlip(enemyIdx, cloneIdx, preset)
    for blipIdx,blip in pairs(blips) do
...
function LDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(blip, add, ignoreGrouped,pulls,pull,ignoreUpdates)
...
function LDT:DungeonEnemies_UpdateBlipColors(pull, r, g, b,pulls)
...
function LDT:DungeonEnemies_UpdateSelected(pull,pulls)
    preset = LDT:GetCurrentPreset()
    pulls = pulls or preset.value.pulls
    --deselect all
    for _,blip in pairs(blips) do
    ...
    for pullIdx,p in pairs(pulls) do
        local r,g,b = LDT:DungeonEnemies_GetPullColor(pullIdx)
        for enemyIdx,clones in pairs(p) do
            if tonumber(enemyIdx) then
                for _,cloneIdx in pairs(clones) do
                    for _,blip in pairs(blips) do
    ...
    LDT:DrawAllHulls(pulls)
end

---DungeonEnemies_SetPullColor
---Sets a custom color for a pull
function LDT:DungeonEnemies_SetPullColor(pull, r, g, b)
    preset = LDT:GetCurrentPreset()
    preset.value.pulls[pull]["color"] = LDT:RGBToHex(r,g,b)
end

---DungeonEnemies_GetPullColor
---Returns the custom color for a pull
function LDT:DungeonEnemies_GetPullColor(pull,pulls)
    pulls = pulls or preset.value.pulls
    local r,g,b = LDT:HexToRGB(pulls[pull]["color"])
    if not r then
        r,g,b = LDT:HexToRGB(db.defaultColor)
        LDT:DungeonEnemies_SetPullColor(pull,r,g,b)
    end
    return r,g,b
end
...
function LDT:DungeonEnemies_UpdateTeeming()
    preset = LDT:GetCurrentPreset()
...
function LDT:DungeonEnemies_UpdateSeasonalAffix()
...
function LDT:DungeonEnemies_UpdateBlacktoothEvent()
    local week
...
function LDT:DungeonEnemies_UpdateBoralusFaction(faction)
    preset = LDT:GetCurrentPreset()
    local teeming = LDT:IsPresetTeeming(preset)
    for _,blip in pairs(blips) do
...
function LDT:DungeonEnemies_UpdateReaping()
    for _,blip in pairs(blips) do
...
function LDT:DungeonEnemies_UpdateInfested(week)
    week = preset.week%3
    if week == 0 then week = 3 end
    for _,blip in pairs(blips) do
...
function LDT:DungeonEnemies_UpdateInspiring(week)
    week = week or preset.week
    local isInspiring = LDT:IsWeekInspiring(week)
    for _,blip in pairs(blips) do
...
function LDT:DungeonEnemies_UpdateFreeholdCrew(crewIdx)
    ...
    local crew = LDT.freeholdCrews[crewIdx]
    for _,blip in pairs(blips) do
        ...
        if enemy.id == poi.npcId then
            ...
            for cloneIdx,clone in pairs(enemy.clones) do
                if clone.week[week] then
                    return LDT:IsCloneInPulls(enemyIdx,cloneIdx)
                end
            end
        end
    end
...
function LDT:IsCloneInPulls(enemyIdx, cloneIdx)
    local pulls = LDT:GetCurrentPreset().value.pulls
    local numClones = 0
...
function LDT:GetEnemyForces(npcId)
...
function LDT:GetNPCNameById(npcId)
    local data = LDT.dungeonEnemies[db.currentDungeonIdx]
    if data then
...
function LDT:UpdateDungeonData(dungeonData)

    local function printDungeonName(shouldPrint,dungeonIdx)
        if shouldPrint then
            print("-----", LDT:GetDungeonName(dungeonIdx))
        end
        return false
    end
    ...
    for dungeonIdx,newData in pairs(dungeonData) do
        ...
        local totalCount = LDT.dungeonTotalCount[dungeonIdx]
        ...
        local enemyData = LDT.dungeonEnemies[dungeonIdx]
        if enemyData then
...
--]
function LDT:ExportNPCIdsWithoutDisplayIds()
    local output = "dungeons = [\n"
    for idx = 15, LDT:GetNumDungeons() do
        local shouldAddDungeonText = true
        local enemyData = LDT.dungeonEnemies[idx]
        if enemyData then
            for _,enemy in pairs(enemyData) do
                if not enemy.displayId then
                    if shouldAddDungeonText then
                        output = output.."Dungeon(name='".. LDT:GetDungeonName(idx).."', idx=".. idx..", npcIds=["
    ...
    LDT.main_frame.ExportFrameEditbox:SetLabel("NPC ids without displayId")
end
...
function LDT:CleanEnemyData(dungeonIdx)
    local enemies = LDT.dungeonEnemies[dungeonIdx]
