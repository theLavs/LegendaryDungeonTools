...
3. Call LDT:AddNPCFromUnit("mouseover") with macro to add untargetable units
...
function LDT:AddNPCFromUnit(unit)
    db = LDT:GetDB()
    local npcId
    ...
    local added
    for _,npcData in pairs(LDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    if npcId and not added then
        ...
        local npcScale = 1
        local npcCount = 0
        tinsert(LDT.dungeonEnemies[db.currentDungeonIdx], {
            name = npcName,
            health = npcHealth,
            level = npcLevel,
...
function LDT:CreateDevPanel(frame)
    db = LDT:GetDB()
    ...
    LDT:FixAceGUIShowHide(devPanel)
    ...
    local function DrawGroup1(container)
        ...
        local buttons = {
            [1] = {
                text="MapLink",
                func=function()
                    if not LDT.mapPOIs[db.currentDungeonIdx] then LDT.mapPOIs[db.currentDungeonIdx] = {} end
                    if not LDT.mapPOIs[db.currentDungeonIdx][LDT:GetCurrentSubLevel()] then
                        LDT.mapPOIs[db.currentDungeonIdx][LDT:GetCurrentSubLevel()] = {}
                    end
                    local links = LDT.mapPOIs[db.currentDungeonIdx][LDT:GetCurrentSubLevel()]
            ...
            [7] = {
                text="Export to LUA",
                func=function()
                    local export = tshow(LDT.mapPOIs[db.currentDungeonIdx],"LDT.mapPOIs[dungeonIndex]")
                    ...
                    LDT.main_frame.ExportFrameEditbox:HighlightText(0, slen(export))
    ...
    local function DrawGroup2(container)
        ...
        local function updateFields(health,level,creatureType,id,scale,count,idx)
            if idx then
                local data = LDT.dungeonEnemies[db.currentDungeonIdx][idx]
        ...
        local function updateDropdown(npcId,idx)
            if not LDT.dungeonEnemies[db.currentDungeonIdx] then return end
            idx = idx or 1
            local enemies = {}
            for mobIdx,data in ipairs(LDT.dungeonEnemies[db.currentDungeonIdx]) do
        ...
        dropdown:SetCallback("OnValueChanged", function(widget,callbackName,key)
            ...
            local dungeonEnemyBlips = LDT:GetDungeonEnemyBlips()
        ...
        countSlider:SetCallback("OnMouseUp",function(widget,callbackName,value)
            ...
            local data = LDT.dungeonEnemies[db.currentDungeonIdx][npcIdx]
            data["count"] = value
            LDT:UpdateMap()
        end)
        ...
        button1:SetCallback("OnClick",function()
            local npcId = LDT:AddNPCFromUnit("target")
        ...
        button2:SetCallback("OnClick",function()
            local currentBlip = LDT:GetCurrentDevmodeBlip()
            if currentBlip then
                --encounterID
                local encounterID, encounterName, description, displayInfo, iconImage = EJ_GetCreatureInfo(1)
                if not encounterID then
                    print("LDT: Error - Make sure to open Encounter Journal and navigate to the boss you want to add!")
                    return
                end
                for i=1,10000 do
                ...
                local data = LDT.dungeonEnemies[db.currentDungeonIdx][currentBlip.enemyIdx]
                data.isBoss = true
        ...
        button3:SetCallback("OnClick",function()
            LDT:CleanEnemyData(db.currentDungeonIdx)
            local export = tshow(LDT.dungeonEnemies[db.currentDungeonIdx],"LDT.dungeonEnemies[dungeonIndex]")
            LDT.main_frame.ExportFrame:ClearAllPoints()
            LDT.main_frame.ExportFrame:Show()
            LDT.main_frame.ExportFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
            LDT.main_frame.ExportFrameEditbox:SetText(export)
        ...
        button4:SetCallback("OnClick",function()
            local dungeons = {}
            for i = 29,36 do
                LDT:CleanEnemyData(i)
                dungeons[i] = LDT.dungeonEnemies[i]
            end
            local export = LDT:TableToString(dungeons,true,5)
            LDT.main_frame.ExportFrame:ClearAllPoints()
            LDT.main_frame.ExportFrame:Show()
            LDT.main_frame.ExportFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
            LDT.main_frame.ExportFrameEditbox:SetText(export)
    ...
    local function DrawGroup5(container)
        ...
        createSpireButton:SetCallback("OnClick",function()
            ...
            local pois = LDT.mapPOIs[db.currentDungeonIdx][LDT:GetCurrentSubLevel()]
            local posx,posy = 300,-200
            local newWeek = LDT:DeepCopy(week)
            ...
            newWeek = LDT:DeepCopy(week)
            LDT:POI_UpdateAll()
            --add associated NPC to the map
            LDT:AddCloneFromData(tooltipIdxToNpcId[tooltipIndex],newWeek)
        end)
    ...
    local function DrawGroup6(container)
        ...
        resetDbButton:SetCallback("OnClick",function()
            LDT:OpenConfirmationFrame(300, 150, "Reset LDT DB", "Confirm", "Do you want to reset LDT DB?", function()
                LDT:HardReset()
            end, "Cancel", nil)
        end)
    ...
    --hook UpdateMap
    local originalFunc = LDT.UpdateMap
    function LDT:UpdateMap(...)
        originalFunc(...)
        local selectedTab
...
function LDT:AddCloneFromData(npcId, weeks)
    local sublevel= LDT:GetCurrentSubLevel()
    local x,y = 320,-200
    local data
    for _,enemyData in pairs(LDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    print(string.format("LDT: Created clone %s %d at %d,%d",data.name,#data.clones,x,y))
    LDT:UpdateMap()
end
...
function LDT:AddCloneAtCursorPosition()
    if not MouseIsOver(LDTScrollFrame) then return end
    if currentEnemyIdx then
        local data = LDT.dungeonEnemies[db.currentDungeonIdx][currentEnemyIdx]
        local cursorx,cursory = LDT:GetCursorPosition()
        local scale = self:GetScale()
        cursorx = cursorx*(1/scale)
        cursory = cursory*(1/scale)
        tinsert(data.clones,{ x=cursorx, y=cursory, sublevel= LDT:GetCurrentSubLevel(), g=currentCloneGroup, teeming=currentTeeming})
        print(string.format("LDT: Created clone %s %d at %d,%d",data.name,#data.clones,cursorx,cursory))
        LDT:UpdateMap()
    end
end
...
function LDT:AddPatrolWaypointAtCursorPosition()
    if not MouseIsOver(LDTScrollFrame) then return end
    local currentBlip = LDT:GetCurrentDevmodeBlip()
    if currentBlip then
        local data = LDT.dungeonEnemies[db.currentDungeonIdx][currentBlip.enemyIdx]
        ...
        local patrolBlips = LDT:GetPatrolBlips()
        ...
        print(string.format("LDT: Created Waypoint %d of %s %d at %d,%d",1,data.name,#cloneData.patrol,cursorx,cursory))
        LDT:UpdateMap()
    end
end
