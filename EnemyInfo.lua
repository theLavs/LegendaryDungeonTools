...
local function MakeEnemeyInfoFrame()
    ...
    f.frame:SetAllPoints(LDTScrollFrame)

    local originalHide = LDT.main_frame.Hide
    function LDT.main_frame:Hide(...)
        f.frame:Hide()
    ...
    local function DrawGroup1(container)
        ...
        LDT:FixAceGUIShowHide(model,modelContainer.frame,true)
        ...
        sendSpellsButton:SetCallback("OnClick",function()
            ...
            SendChatMessage(string.format(L["LDT: Spells for %s:"],enemyName),distribution)
...
local lastEnemyIdx
function LDT:UpdateEnemyInfoFrame(enemyIdx)
    if not enemyIdx then enemyIdx = lastEnemyIdx end
    lastEnemyIdx = enemyIdx
    if not enemyIdx then return end
    local data = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]
    local f = LDT.EnemyInfoFrame
    ...
    local enemies = {}
    for mobIdx,edata in ipairs(LDT.dungeonEnemies[db.currentDungeonIdx]) do
    ...
    LDT:UpdateEnemyInfoData(enemyIdx)
    ...
    if data.spells then
        for spellId,spellData in pairs(data.spells) do
            if not spellBlacklist[spellId] then
                local spellButton = AceGUI:Create("LDTSpellButton")
...
function LDT:UpdateEnemyInfoData(enemyIdx)
    local f = LDT.EnemyInfoFrame
    if not enemyIdx then enemyIdx = lastEnemyIdx end
    if not enemyIdx then return end
    local data = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]
    ...
    local healthText = LDT:FormatEnemyHealth(health)
    ...
    local fortifiedTyrannical = LDT:IsCurrentPresetFortified() and L["Fortified"] or L["Tyrannical"]
    f.enemyDataContainer.healthEditBox:SetLabel(string.format(L["Enemy Info NPC Health"],level,fortifiedTyrannical))
end

function LDT:ShowEnemyInfoFrame(blip)
    db = LDT:GetDB()
    LDT.EnemyInfoFrame = LDT.EnemyInfoFrame or MakeEnemeyInfoFrame()
    LDT:UpdateEnemyInfoFrame(blip.enemyIdx)
    LDT.EnemyInfoFrame:Show()
end
