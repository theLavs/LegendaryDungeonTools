...
LDT.BackdropColor = { 0.058823399245739, 0.058823399245739, 0.058823399245739, 0.9}

local AceGUI = LibStub("AceGUI-3.0")
local db
local icon = LibStub("LibDBIcon-1.0")
local LDB = LibStub("LibDataBroker-1.1"):NewDataObject("legendarydungeontools", {
	type = "data source",
	text = "Legendary Dungeon Tools",
	...
	OnTooltipShow = function(tooltip)
		...
		tooltip:AddLine(mythicColor .."Legendary Dungeon Tools|r")
		tooltip:AddLine(L["Click to toggle AddOn Window"])
		tooltip:AddLine(L["Right-click to lock Minimap Button"])
	end,
})

SLASH_LEGENDARYDUNGEONTOOLS1 = "/mplus"
SLASH_LEGENDARYDUNGEONTOOLS2 = "/ldt"
SLASH_LEGENDARYDUNGEONTOOLS3 = "/legendarydungeontools"

BINDING_NAME_LDTTOGGLE = L["Toggle Window"]
BINDING_NAME_LDTNPC = L["New NPC at Cursor Position"]
BINDING_NAME_LDTWAYPOINT = L["New Patrol Waypoint at Cursor Position"]

function SlashCmdList.legendarydungeontools(cmd, editbox)
	local rqst, arg = strsplit(' ', cmd)
	if rqst == "devmode" then
		LDT:ToggleDevMode()
	elseif rqst == "reset" then
        LDT:ResetMainFramePos()
	elseif rqst == "dc" then
        LDT:ToggleDataCollection()
    elseif rqst == "hptrack" then
        LDT:ToggleHealthTrack()
    else
		LDT:ShowInterface()
	end
end

function LDT:GetLocaleIndex()
    local localeToIndex = {
        ["enUS"] = 1,
        ["deDE"] = 2,
        ["esES"] = 3,
...
do
    ...
    frame:SetScript("OnEvent", function(self, event, ...)
        return LDT[event](self,...)
    end)

    function LDT.ADDON_LOADED(self, addon)
        if addon == "legendarydungeontools" then
			db = LibStub("AceDB-3.0"):New("legendarydungeontoolsDB", defaultSavedVars).global
			icon:Register("mythicdungeontools", LDB, db.minimap)
			if not db.minimap.hide then
				icon:Show("mythicdungeontools")
			end

            --if db.dataCollectionActive then LDT.DataCollection:Init() end
            --fix db corruption
            do
    ...
    local last = 0
    function LDT.GROUP_ROSTER_UPDATE(self, addon)
        --check not more than once per second (blizzard event spam)
        local now = GetTime()
        if last < now - 1 then
            if not LDT.main_frame then return end
            local inGroup = UnitInRaid("player") or IsInGroup()
            LDT.main_frame.LinkToChatButton:SetDisabled(not inGroup)
            LDT.main_frame.LiveSessionButton:SetDisabled(not inGroup)
            if inGroup then
                ...
                if LDT.liveSessionActive then
                    ...
                    LDT.main_frame.LiveSessionButton.text:SetTextColor(0,1,0)
                else
                    LDT.main_frame.LiveSessionButton:SetText(L["Live"])
                    LDT.main_frame.LiveSessionButton.text:SetTextColor(1,0.8196,0)
                end
            else
                LDT.main_frame.LinkToChatButton.text:SetTextColor(0.5,0.5,0.5)
                LDT.main_frame.LiveSessionButton.text:SetTextColor(0.5,0.5,0.5)
            end
            last = now
            --LDT:RequestDataCollectionUpdate()
        end
    end
    function LDT.PLAYER_ENTERING_WORLD(self, addon)
...
function LDT:GetNumDungeons() return #dungeonList-1 end
function LDT:GetDungeonName(idx) return dungeonList[idx] end
...
function LDT:GetDungeonSublevels()
    return dungeonSubLevels
end

function LDT:GetSublevelName(dungeonIdx, sublevelIdx)
...
function LDT:ShowInterface(force)
	...
	if self.main_frame:IsShown() and not force then
        ...
        --edge case if user closed LDT window while in the process of dragging a corrupted blip
        if self.draggedBlip then
            if LDT.liveSessionActive then
                LDT:LiveSession_SendCorruptedPositions(LDT:GetRiftOffsets())
            end
            self:UpdateMap()
            self.draggedBlip = nil
        end
        LDT:UpdateBottomText()
        if not next(db.dungeonImport) then
            LDT:OpenNoDungeonDataWarning()
        end
	end
end

function LDT:HideInterface()
	self.main_frame:Hide()
	self.main_frame.HelpButton:Hide()
end

function LDT:ToggleDevMode()
    db.devMode = not db.devMode
    ReloadUI()
end

function LDT:ToggleDataCollection()
    db.dataCollectionActive = not db.dataCollectionActive
    print(string.format("%sLDT|r: DataCollection %s. Reload Interface!", mythicColor,db.dataCollectionActive and "|cFF00FF00Enabled|r" or "|cFFFF0000Disabled|r"))
end

function LDT:ToggleHealthTrack()
    LDT.DataCollection:InitHealthTrack()
    print(string.format("%sLDT|r: HealthTrack %s.", mythicColor,"|cFF00FF00Enabled|r"))
end


function LDT:CreateMenu()
    -- Close button
    self.main_frame.closeButton = CreateFrame("Button", "LDTCloseButton", self.main_frame, "UIPanelCloseButton")
    ...
    --Maximize Button
    self.main_frame.maximizeButton = CreateFrame("Button", "LDTMaximizeButton", self.main_frame, "MaximizeMinimizeButtonFrameTemplate")
    ...
    self.main_frame.liveReturnButton = CreateFrame("Button", "LDTLiveReturnButton", self.main_frame, "UIPanelCloseButton")
    ...
    self.main_frame.setLivePresetButton = CreateFrame("Button", "LDTSetLivePresetButton", self.main_frame, "UIPanelCloseButton")
...
function LDT:GetDefaultMapPanelSize()
    return sizex,sizey
end

---GetScale
---Returns scale factor stored in db
function LDT:GetScale()
...
function LDT:GetFullScreenSizes()
...
---Maximize
---FULLSCREEN the UI
function LDT:Maximize()
    local f = LDT.main_frame
    ...
    if not f.blackoutFrame then
        f.blackoutFrame = CreateFrame("Frame", "LDTBlackoutFrame", f)
    ...
    local newSizex, newSizey, scale, isNarrow = LDT:GetFullScreenSizes()
    db.scale = scale
    f:ClearAllPoints()
    ...
    f.resizer:Hide()
    LDT:CreateTutorialButton(LDT.main_frame)
    db.maximized = true
end

---Minimize
---Restore normal UI
function LDT:Minimize()
    local f = LDT.main_frame
...
function LDT:SkinProgressBar(progressBar)
...
function LDT:IsFrameOffScreen()
    local topPanel = LDT.main_frame.topPanel
    local bottomPanel = LDT.main_frame.bottomPanel
...
local bottomTips = {
    [1] = L["Please report any bugs on https://github.com/Nnoggie/legendarydungeontools/issues"],
    ...
    [7] = L["Drag the bottom right edge to resize LDT."],
    [8] = L["Click the fullscreen button for a maximized view of LDT."],
    [9] = L["Use /LDT reset to restore the default position and scale of LDT."],
    [10] = L["Mouseover the Live button while in a group to learn more about Live mode."],
    [11] = L["You are using LDT. You rock!"],
...
function LDT:UpdateBottomText()
    local f = self.main_frame.bottomPanelString
    f:SetText(bottomTips[math.random(#bottomTips)])
end

function LDT:MakeTopBottomTextures(frame)
    frame:SetMovable(true)
	if frame.topPanel == nil then
		frame.topPanel = CreateFrame("Frame", "LDTTopPanel", frame)
		frame.topPanelTex = frame.topPanel:CreateTexture(nil, "BACKGROUND")
		frame.topPanelTex:SetAllPoints()
		frame.topPanelTex:SetDrawLayer(canvasDrawLayer, -5)
		frame.topPanelTex:SetColorTexture(unpack(LDT.BackdropColor))
		frame.topPanelString = frame.topPanel:CreateFontString("LDT name")
    ...
    frame.topPanel:SetScript("OnDragStop", function(self,button)
        ...
        if LDT:IsFrameOffScreen() then
            LDT:ResetMainFramePos(true)
        else
            local from,_,to,x,y = LDT.main_frame:GetPoint()
            db.anchorFrom = from
            db.anchorTo = to
    ...
    if frame.bottomPanel == nil then
        frame.bottomPanel = CreateFrame("Frame", "LDTBottomPanel", frame)
        ...
        frame.bottomPanelTex:SetColorTexture(unpack(LDT.BackdropColor))
    end
    ...
    frame.bottomPanelString = frame.bottomPanel:CreateFontString("LDTMid")
    ...
    frame.bottomLeftPanelString = frame.bottomPanel:CreateFontString("LDTVersion")
...
function LDT:MakeSidePanel(frame)

	if frame.sidePanel == nil then
		frame.sidePanel = CreateFrame("Frame", "LDTSidePanel", frame)
		...
		frame.sidePanelTex:SetColorTexture(unpack(LDT.BackdropColor))
	...
	frame.sidePanelString = frame.sidePanel:CreateFontString("LDTSidePanelText")
	...
	dropdown:SetCallback("OnValueChanged",function(widget,callbackName,key)
		if db.presets[db.currentDungeonIdx][key].value==0 then
			LDT:OpenNewPresetDialog()
			LDT.main_frame.sidePanelDeleteButton:SetDisabled(true)
			LDT.main_frame.sidePanelDeleteButton.text:SetTextColor(0.5,0.5,0.5)
		else
			if key == 1 then
				LDT.main_frame.sidePanelDeleteButton:SetDisabled(true)
                LDT.main_frame.sidePanelDeleteButton.text:SetTextColor(0.5,0.5,0.5)
			else
                if not LDT.liveSessionActive then
                    LDT.main_frame.sidePanelDeleteButton:SetDisabled(false)
                    LDT.main_frame.sidePanelDeleteButton.text:SetTextColor(1,0.8196,0)
                else
                    LDT.main_frame.sidePanelDeleteButton:SetDisabled(true)
                    LDT.main_frame.sidePanelDeleteButton.text:SetTextColor(0.5,0.5,0.5)
                end
            ...
            --frame.sidePanel.affixDropdown:SetAffixWeek(LDT:GetCurrentPreset().week or LDT:GetCurrentAffixWeek())
			--UpdateMap is called in SetAffixWeek, no need to call twice
            LDT:UpdateMap()
            frame.sidePanel.affixDropdown:SetAffixWeek(LDT:GetCurrentPreset().week or LDT:GetCurrentAffixWeek() or 1)
		end
	end)
	LDT:UpdatePresetDropDown()
	...
	--button fontInstance
	local fontInstance = CreateFont("LDTButtonFont")
	...
	frame.sidePanelRenameButton:SetCallback("OnClick",function(widget,callbackName,value)
		...
		LDT.main_frame.RenameFrame:Show()
		LDT.main_frame.RenameFrame.RenameButton:SetDisabled(true)
		LDT.main_frame.RenameFrame.RenameButton.text:SetTextColor(0.5,0.5,0.5)
        LDT.main_frame.RenameFrame:ClearAllPoints()
		LDT.main_frame.RenameFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
		LDT.main_frame.RenameFrame.Editbox:SetText(currentPresetName)
		LDT.main_frame.RenameFrame.Editbox:HighlightText(0, string.len(currentPresetName))
		LDT.main_frame.RenameFrame.Editbox:SetFocus()
	end)
    ...
    frame.sidePanelImportButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine(L["You can find LDT exports from other users on the wago.io website"],1,1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelExportButton:SetCallback("OnClick",function(widget,callbackName,value)
        if db.colorPaletteInfo.forceColorBlindMode then LDT:ColorAllPulls(_,_,_,true) end
        local preset = LDT:GetCurrentPreset()
        LDT:SetUniqueID(preset)
        preset.mdiEnabled = db.MDI.enabled
        preset.difficulty = db.currentDifficulty
		local export = LDT:TableToString(preset,true,5)
		LDT:HideAllDialogs()
		LDT.main_frame.ExportFrame:Show()
        LDT.main_frame.ExportFrame:ClearAllPoints()
		LDT.main_frame.ExportFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
		LDT.main_frame.ExportFrameEditbox:SetText(export)
		LDT.main_frame.ExportFrameEditbox:HighlightText(0, string.len(export))
		LDT.main_frame.ExportFrameEditbox:SetFocus()
        LDT.main_frame.ExportFrameEditbox:SetLabel(preset.text.." "..string.len(export))
        if db.colorPaletteInfo.forceColorBlindMode then LDT:ColorAllPulls() end
    end)
    frame.sidePanelExportButton.frame:SetScript("OnEnter",function()
        ...
        GameTooltip:AddLine(L["You can share LDT exports on the wago.io website"],1,1,1,1)
        GameTooltip:Show()
    end)
	...
	frame.sidePanelDeleteButton:SetCallback("OnClick",function(widget,callbackName,value)
        if IsShiftKeyDown() then
            ...
            LDT:OpenConfirmationFrame(450,150,L["Delete ALL presets"],L["Delete"],prompt, LDT.DeleteAllPresets)
        else
            LDT:HideAllDialogs()
            frame.DeleteConfirmationFrame:ClearAllPoints()
            frame.DeleteConfirmationFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
            local currentPresetName = db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].text
	...
	frame.LinkToChatButton:SetCallback("OnClick",function(widget,callbackName,value)
        ...
        if presetSize>25000 then
            ...
            LDT:OpenConfirmationFrame(450,150,L["Sharing large preset"],"Share",prompt, callback)
        else
            callback()
        end
	end)
    ...
    frame.AutomaticColorsCheckSidePanel:SetCallback("OnValueChanged",function(widget,callbackName,value)
        ...
        if value == true then
            ...
            LDT:ColorAllPulls()
            LDT.main_frame.AutomaticColorsCogwheel:SetImage("Interface\\AddOns\\legendarydungeontools\\Textures\\helpIconRnbw")
        else
            frame.toggleForceColorBlindMode:SetDisabled(true)
            LDT.main_frame.AutomaticColorsCogwheel:SetImage("Interface\\AddOns\\legendarydungeontools\\Textures\\helpIconGrey")
        end
	end)
    --AutomaticColorsCogwheel
    frame.AutomaticColorsCogwheel = AceGUI:Create("Icon")
    local colorCogwheel = frame.AutomaticColorsCogwheel
    colorCogwheel:SetImage("Interface\\AddOns\\legendarydungeontools\\Textures\\helpIconRnbw")
    ...
    local function makeAffixString(week,affixes,longText)
        ...
        for _,affixID in ipairs(affixes) do
            ...
            if longText then
                ...
                if week == LDT:GetCurrentAffixWeek() then
    ...
    function affixDropdown:SetAffixWeek(key,ignoreReloadPullButtons,ignoreUpdateProgressBar)
        ...
        elseif LDT:GetCurrentAffixWeek() == key then
            frame.sidePanel.affixWeekWarning.image:Hide()
            frame.sidePanel.affixWeekWarning:SetDisabled(true)
        else
        ...
        LDT:GetCurrentPreset().week = key
        local teeming = LDT:IsPresetTeeming(LDT:GetCurrentPreset())
        LDT:GetCurrentPreset().value.teeming = teeming

        if LDT.EnemyInfoFrame and LDT.EnemyInfoFrame.frame:IsShown() then LDT:UpdateEnemyInfoData() end
        LDT:UpdateMap(nil,ignoreReloadPullButtons,ignoreUpdateProgressBar)
    end
    affixDropdown:SetCallback("OnValueChanged",function(widget,callbackName,key)
        ...
        if LDT.liveSessionActive and LDT:GetCurrentPreset().uid == LDT.livePresetUID then
            LDT:LiveSession_SendAffixWeek(key)
        end
    end)
    ...
    affixWeekWarning:SetCallback("OnClick",function(...)
        if not LDT:GetCurrentAffixWeek() then return end
        affixDropdown:SetAffixWeek(LDT:GetCurrentAffixWeek())
        if LDT.liveSessionActive and LDT:GetCurrentPreset().uid == LDT.livePresetUID then
            LDT:LiveSession_SendAffixWeek(LDT:GetCurrentAffixWeek())
        end
    end)
	...
	frame.sidePanel.DifficultySlider:SetCallback("OnValueChanged",function(widget,callbackName,value)
        ...
        LDT:GetCurrentPreset().difficulty = db.currentDifficulty
        ...
        timer = C_Timer.NewTimer(2, function()
            ...
            if LDT.liveSessionActive then
                local livePreset = LDT:GetCurrentLivePreset()
                local shouldUpdate = livePreset == LDT:GetCurrentPreset()
                if shouldUpdate then LDT:LiveSession_SendDifficulty() end
            end
        end)
	end)
    ...
    LDT:SkinProgressBar(frame.sidePanel.ProgressBar)
end

---ToggleMDIMode
---Enables display to override beguiling+freehold week
function LDT:ToggleMDIMode()
...
function LDT:DisplayMDISelector()
    local show = db.MDI.enabled
    db = LDT:GetDB()
    if not LDT.MDISelector then
        LDT.MDISelector = AceGUI:Create("SimpleGroup")
        LDT.MDISelector.frame:SetFrameStrata("HIGH")
        LDT.MDISelector.frame:SetFrameLevel(50)
        if not LDT.MDISelector.frame.SetBackdrop then
            Mixin(LDT.MDISelector.frame, BackdropTemplateMixin)
        end
        LDT.MDISelector.frame:SetBackdropColor(unpack(LDT.BackdropColor))
        --fix show hide
        local frame = LDT.main_frame
        local originalShow,originalHide = frame.Show,frame.Hide
        local widget = LDT.MDISelector.frame
        function frame:Hide(...)
            widget:Hide()
            return originalHide(self, ...)
        end
        ...
        LDT.MDISelector:SetLayout("Flow")
        LDT.MDISelector.frame.bg = LDT.MDISelector.frame:CreateTexture(nil, "BACKGROUND")
        LDT.MDISelector.frame.bg:SetAllPoints(LDT.MDISelector.frame)
        LDT.MDISelector.frame.bg:SetColorTexture(unpack(LDT.BackdropColor))
        LDT.MDISelector:SetWidth(145)
        LDT.MDISelector:SetHeight(90)
        LDT.MDISelector.frame:ClearAllPoints()
        LDT.MDISelector.frame:SetPoint("BOTTOMRIGHT", LDT.main_frame,"BOTTOMRIGHT",0,0)

        local label = AceGUI:Create("Label")
        label:SetText(L["MDI Mode"])
        LDT.MDISelector:AddChild(label)

        --beguiling
        LDT.MDISelector.BeguilingDropDown = AceGUI:Create("Dropdown")
        LDT.MDISelector.BeguilingDropDown:SetLabel(L["Seasonal Affix:"])
        ...
        LDT.MDISelector.BeguilingDropDown:SetList(beguilingList)
        LDT.MDISelector.BeguilingDropDown:SetCallback("OnValueChanged",function(widget, callbackName, key)
        ...
        LDT.MDISelector:AddChild(LDT.MDISelector.BeguilingDropDown)

        --freehold
        LDT.MDISelector.FreeholdDropDown = AceGUI:Create("Dropdown")
        LDT.MDISelector.FreeholdDropDown:SetLabel(L["Freehold:"])
        local freeholdList = {string.format("1. %s",L["Cutwater"]),string.format("2. %s",L["Blacktooth"]),string.format("3. %s",L["Bilge Rats"])}
        LDT.MDISelector.FreeholdDropDown:SetList(freeholdList)
        LDT.MDISelector.FreeholdDropDown:SetCallback("OnValueChanged",function(widget, callbackName, key)
            local preset = LDT:GetCurrentPreset()
        ...
        LDT.MDISelector:AddChild(LDT.MDISelector.FreeholdDropDown)

        LDT.MDISelector.FreeholdCheck = AceGUI:Create("CheckBox")
        LDT.MDISelector.FreeholdCheck:SetLabel(L["Join Crew"])
        LDT.MDISelector.FreeholdCheck:SetCallback("OnValueChanged",function(widget, callbackName, value)
            local preset = LDT:GetCurrentPreset()
        ...
        LDT.MDISelector:AddChild(LDT.MDISelector.FreeholdCheck)
    ...
    if show then
        ...
        db.currentSeason = LDT:GetEffectivePresetSeason(preset)
        LDT:DungeonEnemies_UpdateSeasonalAffix()
        LDT:DungeonEnemies_UpdateBoralusFaction(LDT:GetCurrentPreset().faction)
        --freehold
        ...
        LDT.MDISelector.FreeholdCheck:SetValue(preset.mdi.freeholdJoined)
        LDT:DungeonEnemies_UpdateFreeholdCrew()
        LDT:DungeonEnemies_UpdateBlacktoothEvent()
        LDT:UpdateProgressbar()
        LDT:ReloadPullButtons()
        LDT.MDISelector.frame:Show()
        LDT:ToggleFreeholdSelector(false)
    else
        db.currentSeason = defaultSavedVars.global.currentSeason
        LDT:DungeonEnemies_UpdateSeasonalAffix()
        LDT:DungeonEnemies_UpdateBoralusFaction(LDT:GetCurrentPreset().faction)
        LDT:UpdateFreeholdSelector(LDT:GetCurrentPreset().week)
        LDT:DungeonEnemies_UpdateBlacktoothEvent()
        LDT:UpdateProgressbar()
        LDT:ReloadPullButtons()
        LDT.MDISelector.frame:Hide()
        LDT:ToggleFreeholdSelector(db.currentDungeonIdx == 16)
    end
    LDT:POI_UpdateAll()
    LDT:KillAllAnimatedLines()
    LDT:DrawAllAnimatedLines()
...
function LDT:UpdatePresetDropDown()
	local dropdown = LDT.main_frame.sidePanel.WidgetGroup.PresetDropDown
	local presetList = {}
...
function LDT:UpdatePresetDropdownTextColor(forceReset)
    local preset = self:GetCurrentPreset()
...
---FormatEnemyForces
function LDT:FormatEnemyForces(forces, forcesmax, progressbar)
    if not forcesmax then forcesmax = LDT:IsCurrentPresetTeeming() and LDT.dungeonTotalCount[db.currentDungeonIdx].teeming or LDT.dungeonTotalCount[db.currentDungeonIdx].normal end
...
function LDT:Progressbar_SetValue(self, totalCurrent, totalMax)
	...
	self.Bar.Label:SetText(LDT:FormatEnemyForces(totalCurrent,totalMax,true))
	self.AnimValue = percent
end

---UpdateProgressbar
---Update the progressbar on the sidepanel with the correct values
function LDT:UpdateProgressbar()
	local teeming = db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].value.teeming
    LDT:EnsureDBTables()
    local grandTotal = LDT:CountForces()
	LDT:Progressbar_SetValue(LDT.main_frame.sidePanel.ProgressBar,grandTotal,teeming==true and LDT.dungeonTotalCount[db.currentDungeonIdx].teeming or LDT.dungeonTotalCount[db.currentDungeonIdx].normal)
end

function LDT:OnPan(cursorX, cursorY)
    local scrollFrame = LDTScrollFrame
    local scale = LDTMapPanelFrame:GetScale()/1.5
...
function LDT:OnPanFadeOut(deltaTime)
    local scrollFrame = LDTScrollFrame
...
function LDT:ExportCurrentZoomPanSettings()
    local mainFrame = LDTMapPanelFrame
    local scrollFrame = LDTScrollFrame

    local zoom = LDTMapPanelFrame:GetScale()
    local panH = LDTScrollFrame:GetHorizontalScroll() / LDT:GetScale()
    local panV = LDTScrollFrame:GetVerticalScroll() / LDT:GetScale()
    ...
    LDT.main_frame.ExportFrame:ClearAllPoints()
    LDT.main_frame.ExportFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
    LDT.main_frame.ExportFrameEditbox:SetText(output)
    LDT.main_frame.ExportFrameEditbox:HighlightText(0, string.len(output))
    LDT.main_frame.ExportFrameEditbox:SetFocus()
    LDT.main_frame.ExportFrameEditbox:SetLabel("Current pan/zoom settings");
end


function LDT:ZoomMapToDefault()
    ...
    local currentMapInfo = LDT.mapInfo[db.currentDungeonIdx]
    if(currentMapInfo and currentMapInfo.viewportPositionOverrides and currentMapInfo.viewportPositionOverrides[currentSublevel])then
        ...
        scrollFrame:SetHorizontalScroll(data.horizontalPan * LDT:GetScale())
        scrollFrame:SetVerticalScroll(data.verticalPan * LDT:GetScale())
...
function LDT:ZoomMap(delta)
    ...
    LDT:SetPingOffsets(newScale)
end

---ActivatePullTooltip
---
function LDT:ActivatePullTooltip(pull)
    local pullTooltip = LDT.pullTooltip
    --[[
...
function LDT:UpdatePullTooltip(tooltip)
    local frame = LDT.main_frame
    ...
    elseif MouseIsOver(frame.sidePanel.newPullButton.frame) then
		...
		if frame.sidePanel.newPullButtons and tooltip.currentPull and frame.sidePanel.newPullButtons[tooltip.currentPull] then
			...
			for k,v in pairs(frame.sidePanel.newPullButtons[tooltip.currentPull].enemyPortraits) do
				if MouseIsOver(v) then
					if v:IsShown() then
                        ...
                        local health = LDT:CalculateEnemyHealth(boss,v.enemyData.baseHealth,db.currentDifficulty,v.enemyData.ignoreFortified)
                        text = text.. string.format(L["%s HP"],LDT:FormatEnemyHealth(health))..newLine

                        local totalForcesMax = LDT:IsCurrentPresetTeeming() and LDT.dungeonTotalCount[db.currentDungeonIdx].teeming or LDT.dungeonTotalCount[db.currentDungeonIdx].normal
                        local count = LDT:IsCurrentPresetTeeming() and v.enemyData.teemingCount or v.enemyData.count
                        text = text..L["Forces"]..": ".. LDT:FormatEnemyForces(count,totalForcesMax,false)
            ...
            local pullForces = LDT:CountForces(tooltip.currentPull,true)
            local totalForces = LDT:CountForces(tooltip.currentPull,false)
            local totalForcesMax = LDT:IsCurrentPresetTeeming() and LDT.dungeonTotalCount[db.currentDungeonIdx].teeming or LDT.dungeonTotalCount[db.currentDungeonIdx].normal

            local text = L["Forces"]..": ".. LDT:FormatEnemyForces(pullForces,totalForcesMax,false)
            text = text.. "\n"..L["Total"]..": ".. LDT:FormatEnemyForces(totalForces,totalForcesMax,true)
...
function LDT:CountForces(currentPull, currentOnly)
                ...
                for enemyIdx,clones in pairs(pull) do
                    if tonumber(enemyIdx) then
                        for k,v in pairs(clones) do
                            if LDT:IsCloneIncluded(enemyIdx,v) then
                                local count = teeming
...
function LDT:IsCloneIncluded(enemyIdx, cloneIdx)
    local preset = LDT:GetCurrentPreset()
    local isCloneBlacktoothEvent = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].blacktoothEvent
    local cloneFaction = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].faction

    local week = self:GetEffectivePresetWeek()

    if db.currentSeason ~= 3 then
        if emissaryIds[LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx].id] then return false end
    elseif db.currentSeason ~= 4 then
        if LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx].corrupted then return false end
    end

    --beguiling weekly configuration
    local weekData = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].week
    if weekData then
    ...
    if not isCloneBlacktoothEvent or isBlacktoothWeek then
        if not (cloneFaction and cloneFaction~= preset.faction) then
            local isCloneTeeming = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].teeming
            local isCloneNegativeTeeming = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].negativeTeeming
            if LDT:IsCurrentPresetTeeming() or ((isCloneTeeming and isCloneTeeming == false) or (not isCloneTeeming)) then
                if not(LDT:IsCurrentPresetTeeming() and isCloneNegativeTeeming) then
                    return true
                end
            end
        end
...
function LDT:IsCurrentPresetTeeming()
...
function LDT:IsCurrentPresetFortified()
    return self:GetCurrentPreset().week%2 == 1
end

---IsCurrentPresetTyrannical
function LDT:IsCurrentPresetTyrannical()
    return not LDT:IsCurrentPresetFortified()
end

---MouseDownHook
function LDT:MouseDownHook()
    return
end

---Handles mouse-down events on the map scrollframe
LDT.OnMouseDown = function(self, button)
	local scrollFrame = LDT.main_frame.scrollFrame
...
LDT.OnMouseUp = function(self, button)
	local scrollFrame = LDT.main_frame.scrollFrame
    ...
    if scrollFrame.oldX==scrollFrame.cursorX or scrollFrame.oldY==scrollFrame.cursorY then
        if button == "RightButton" then
            local x,y = LDT:GetCursorPosition()
            LDT:PingMap(x,y)
            local sublevel = LDT:GetCurrentSubLevel()
            if LDT.liveSessionActive then LDT:LiveSession_SendPing(x,y,sublevel) end
        end
    end
end

---Pings the map
function LDT:PingMap(x, y)
...
function LDT:SetPingOffsets(mapScale)
    local scale = 0.35
    local offset = (10.25/1000)*mapScale
    LDT.ping:SetTransform(offset,offset,0,0,0,0,scale)
end

---SetCurrentSubLevel
---Sets the sublevel of the currently active preset, need to UpdateMap to reflect the change in UI
function LDT:SetCurrentSubLevel(sublevel)
    LDT:GetCurrentPreset().value.currentSublevel = sublevel
end

---GetCurrentPull
---Returns the current pull of the currently active preset
function LDT:GetCurrentPull()
...
function LDT:GetCurrentSubLevel()
	return LDT:GetCurrentPreset().value.currentSublevel
end

---GetCurrentPreset
---Returns the current preset
function LDT:GetCurrentPreset()
    return db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]]
end

---GetCurrentLivePreset
function LDT:GetCurrentLivePreset()
...
function LDT:GetEffectivePresetWeek(preset)
...
function LDT:GetEffectivePresetSeason(preset)
...
---ReturnToLivePreset
function LDT:ReturnToLivePreset()
...
function LDT:IsWeekTeeming(week)
    if not week then week = LDT:GetCurrentAffixWeek() or 1 end
    return affixWeeks[week][1] == 5
end

---Returns if the current week has an affix weeks set that includes the inspiring affix
function LDT:IsWeekInspiring(week)
    if not week then week = LDT:GetCurrentAffixWeek() or 1 end
...
function LDT:IsPresetTeeming(preset)
    return LDT:IsWeekTeeming(preset.week)
end

function LDT:GetRiftOffsets()
    local week = self:GetEffectivePresetWeek()
    local preset = self:GetCurrentPreset()
...
function LDT:MakeMapTexture(frame)
    LDT.contextMenuList = {}

    tinsert(LDT.contextMenuList, {
        text = "Close",
        notCheckable = 1,
	...
	if frame.scrollFrame == nil then
		frame.scrollFrame = CreateFrame("ScrollFrame", "LDTScrollFrame",frame)
		...
		frame.scrollFrame:SetScript("OnMouseWheel", function(self, delta)
            if IsControlKeyDown() then
                if not lastModifiedScroll or lastModifiedScroll < GetTime() - 0.1 then
                    ...
                    delta = delta*-1
                    local target = LDT:GetCurrentSubLevel()+delta
                    if dungeonSubLevels[db.currentDungeonIdx][target] then
                        LDT:SetCurrentSubLevel(target)
                        LDT:UpdateMap()
            ...
            elseif IsAltKeyDown() then
                if not lastModifiedScroll or lastModifiedScroll < GetTime() - 0.3 then
                    ...
                    if dungeonList[target] and not ignoredTargets[target] then
                        local group = LDT.main_frame.DungeonSelectionGroup
		...
		frame.scrollFrame:SetScript("OnMouseDown", LDT.OnMouseDown)
		frame.scrollFrame:SetScript("OnMouseUp", LDT.OnMouseUp)


		frame.scrollFrame:SetScript("OnUpdate", function(self,elapsed)
			local x, y = GetCursorPosition()
			LDT:OnPan(x, y)
            LDT:OnPanFadeOut(elapsed)
        end)

		if frame.mapPanelFrame == nil then
			frame.mapPanelFrame = CreateFrame("frame","LDTMapPanelFrame",nil)
		...
		for i=1,12 do
			frame["mapPanelTile"..i] = frame.mapPanelFrame:CreateTexture("LDTmapPanelTile"..i, "BACKGROUND")
        ...
        for i=1,10 do
            for j=1,15 do
                frame["largeMapPanelTile"..i..j] = frame.mapPanelFrame:CreateTexture("LDTLargeMapPanelTile"..i..j, "BACKGROUND")
...
function LDT:CalculateEnemyHealth(boss, baseHealth, level, ignoreFortified)
    local fortified = LDT:IsCurrentPresetFortified()
    local tyrannical = LDT:IsCurrentPresetTyrannical()
	local mult = 1
...
function LDT:ReverseCalcEnemyHealth(unit, level, boss)
...
function LDT:FormatEnemyHealth(amount)
...
function LDT:UpdateDungeonEnemies()
    LDT:DungeonEnemies_UpdateEnemies()
end

function LDT:HideAllDialogs()
	LDT.main_frame.presetCreationFrame:Hide()
	LDT.main_frame.presetImportFrame:Hide()
	LDT.main_frame.ExportFrame:Hide()
	LDT.main_frame.RenameFrame:Hide()
	LDT.main_frame.ClearConfirmationFrame:Hide()
	LDT.main_frame.DeleteConfirmationFrame:Hide()
    LDT.main_frame.automaticColorsFrame.CustomColorFrame:Hide()
    LDT.main_frame.automaticColorsFrame:Hide()
    LDT.main_frame.noDungeonDataWarning:Hide()
    LDT.main_frame.dataImportDialog:Hide()
    if LDT.main_frame.ConfirmationFrame then LDT.main_frame.ConfirmationFrame:Hide() end
end

function LDT:OpenImportPresetDialog()
	LDT:HideAllDialogs()
    LDT.main_frame.presetImportFrame:ClearAllPoints()
	LDT.main_frame.presetImportFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
	LDT.main_frame.presetImportFrame:Show()
	LDT.main_frame.presetImportBox:SetText("")
	LDT.main_frame.presetImportBox:SetFocus()
    LDT.main_frame.presetImportLabel:SetText(nil)
end

function LDT:OpenNewPresetDialog()
	LDT:HideAllDialogs()
	local presetList = {}
	...
	LDT.main_frame.PresetCreationDropDown:SetList(presetList)
	LDT.main_frame.PresetCreationDropDown:SetValue(1)
	LDT.main_frame.PresetCreationEditbox:SetText(L["defaultPresetName"].." "..countPresets+1)
    LDT.main_frame.presetCreationFrame:ClearAllPoints()
	LDT.main_frame.presetCreationFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
	LDT.main_frame.presetCreationFrame:SetStatusText("")
	LDT.main_frame.presetCreationFrame:Show()
	LDT.main_frame.presetCreationCreateButton:SetDisabled(false)
	LDT.main_frame.presetCreationCreateButton.text:SetTextColor(1,0.8196,0)
	LDT.main_frame.PresetCreationEditbox:SetFocus()
	LDT.main_frame.PresetCreationEditbox:HighlightText(0,50)
	LDT.main_frame.presetImportBox:SetText("")
end

function LDT:OpenClearPresetDialog()
    LDT:HideAllDialogs()
    LDT.main_frame.ClearConfirmationFrame:ClearAllPoints()
    LDT.main_frame.ClearConfirmationFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
    local currentPresetName = db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].text
    LDT.main_frame.ClearConfirmationFrame.label:SetText(string.format(L["Reset %s?"],currentPresetName))
    LDT.main_frame.ClearConfirmationFrame:Show()
end

function LDT:OpenAutomaticColorsDialog()
	LDT:HideAllDialogs()
	LDT.main_frame.automaticColorsFrame:ClearAllPoints()
	LDT.main_frame.automaticColorsFrame:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
	LDT.main_frame.automaticColorsFrame:SetStatusText("")
	LDT.main_frame.automaticColorsFrame:Show()
    LDT.main_frame.automaticColorsFrame.CustomColorFrame:Hide()
    if db.colorPaletteInfo.colorPaletteIdx == 6 then
        LDT:OpenCustomColorsDialog()
    end
end

function LDT:OpenCustomColorsDialog(frame)
	LDT:HideAllDialogs()
    LDT.main_frame.automaticColorsFrame:Show() --Not the prettiest way to handle this, but it works.
	LDT.main_frame.automaticColorsFrame.CustomColorFrame:ClearAllPoints()
    LDT.main_frame.automaticColorsFrame.CustomColorFrame:SetPoint("CENTER",264,-7)
	LDT.main_frame.automaticColorsFrame.CustomColorFrame:SetStatusText("")
	LDT.main_frame.automaticColorsFrame.CustomColorFrame:Show()
end

function LDT:UpdateDungeonDropDown()
	local group = LDT.main_frame.DungeonSelectionGroup
...
function LDT:CreateDungeonSelectDropdown(frame)
	...
	group.DungeonDropdown:SetCallback("OnValueChanged",function(widget,callbackName,key)
        if key == 14 then
            ...
            LDT:UpdateDungeonDropDown()
            LDT:UpdateToDungeon(db.currentDungeonIdx)
        elseif key == 27 then
...
function LDT:EnsureDBTables()
    ...
    local preset = LDT:GetCurrentPreset()
    preset.week = preset.week or LDT:GetCurrentAffixWeek()
    ...
    for pullIdx,pull in pairs(preset.value.pulls) do
        for enemyIdx,clones in pairs(pull) do
            ...
            if tonumber(enemyIdx) then
                ...
                if not LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx] then
                    pull[enemyIdx] = nil
                else
                    --only clones
                    for k,v in pairs(clones) do
                        if not LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][v] then
                            clones[k] = nil
                        end
                    end
                end
    ...
    LDT:GetCurrentPreset().week = LDT:GetCurrentPreset().week or LDT:GetCurrentAffixWeek()
    ...
    local maxSublevel = -1
    for _,_ in pairs(LDT.dungeonMaps[db.currentDungeonIdx]) do
    ...
    preset.value.teeming = LDT:IsWeekTeeming(preset.week)
end

function LDT:GetTileFormat(dungeonIdx)
    local mapInfo = LDT.mapInfo[dungeonIdx]
    return mapInfo and mapInfo.tileFormat or 4
end

function LDT:UpdateMap(ignoreSetSelection, ignoreReloadPullButtons, ignoreUpdateProgressBar)
	local mapName
	local frame = LDT.main_frame
	mapName = LDT.dungeonMaps[db.currentDungeonIdx][0]
	LDT:EnsureDBTables()
    local preset = LDT:GetCurrentPreset()
	...
	local fileName = LDT.dungeonMaps[db.currentDungeonIdx][preset.value.currentSublevel]
	local path = "Interface\\WorldMap\\"..mapName.."\\"
    local tileFormat = LDT:GetTileFormat(db.currentDungeonIdx)
	for i=1,12 do
	...
	if (db.currentPreset[db.currentDungeonIdx] == 1 or db.currentPreset[db.currentDungeonIdx] == presetCount) or LDT.liveSessionActive then
		LDT.main_frame.sidePanelDeleteButton:SetDisabled(true)
		LDT.main_frame.sidePanelDeleteButton.text:SetTextColor(0.5,0.5,0.5)
	else
		LDT.main_frame.sidePanelDeleteButton:SetDisabled(false)
    ...
    --live mode
    local livePreset = LDT:GetCurrentLivePreset()
    if LDT.liveSessionActive and preset ~= livePreset then
        LDT.main_frame.liveReturnButton:Show()
        LDT.main_frame.setLivePresetButton:Show()
    else
        LDT.main_frame.liveReturnButton:Hide()
        LDT.main_frame.setLivePresetButton:Hide()
    end
    LDT:UpdatePresetDropdownTextColor()

	if not ignoreSetSelection then LDT:SetSelectionToPull(preset.value.currentPull) end
	LDT:UpdateDungeonDropDown()
    --frame.sidePanel.affixDropdown:SetAffixWeek(LDT:GetCurrentPreset().week,ignoreReloadPullButtons,ignoreUpdateProgressBar)
    frame.sidePanel.affixDropdown:SetValue(LDT:GetCurrentPreset().week)
    LDT:ToggleFreeholdSelector(db.currentDungeonIdx == 16)
    LDT:ToggleBoralusSelector(db.currentDungeonIdx == 19)
    LDT:DisplayMDISelector()
    LDT:DrawAllPresetObjects()
    LDT:KillAllAnimatedLines()
    LDT:DrawAllAnimatedLines()
end
...
function LDT:UpdateToDungeon(dungeonIdx, ignoreUpdateMap, init)
    db.currentExpansion = 1
	...
	LDT:UpdatePresetDropDown()
	if not ignoreUpdateMap then LDT:UpdateMap() end
    LDT:ZoomMapToDefault()
     --Colors the first pull in "Default" presets
    if db.currentPreset[db.currentDungeonIdx] == 1 then LDT:ColorPull() end
end

function LDT:DeletePreset(index)
...
function LDT:ClearPreset(preset, silent)
...
function LDT:CreateNewPreset(name)
	if name == "<New Preset>" then
		LDT.main_frame.presetCreationLabel:SetText(string.format(L["Cannot create preset '%s'"],name))
		LDT.main_frame.presetCreationCreateButton:SetDisabled(true)
		LDT.main_frame.presetCreationCreateButton.text:SetTextColor(0.5,0.5,0.5)
		LDT.main_frame.presetCreationFrame:DoLayout()
		return
	end
	...
	if duplicate == false then
		...
		local startingPointPresetIdx = LDT.main_frame.PresetCreationDropDown:GetValue()-1
		if startingPointPresetIdx>0 then
			db.presets[db.currentDungeonIdx][countPresets] = LDT:CopyObject(db.presets[db.currentDungeonIdx][startingPointPresetIdx])
		...
		LDT.main_frame.presetCreationLabel:SetText(string.format(L["Preset '%s' already exists"],name))
		LDT.main_frame.presetCreationCreateButton:SetDisabled(true)
		LDT.main_frame.presetCreationCreateButton.text:SetTextColor(0.5,0.5,0.5)
		LDT.main_frame.presetCreationFrame:DoLayout()
	end
...
function LDT:SanitizePresetName(text)
...
function LDT:MakeChatPresetImportFrame(frame)
    ...
    importButton:SetCallback("OnClick", function()
        ...
        if LDT:ValidateImportPreset(newPreset) then
            chatImport:Hide()
            LDT:ImportPreset(LDT:DeepCopy(newPreset))
        else
            print(L["LDT: Error importing preset"])
        end
    end)
...
function LDT:OpenChatImportPresetDialog(sender, preset, live)
    LDT:HideAllDialogs()
    local chatImport = LDT.main_frame.chatPresetImportFrame
    chatImport:ClearAllPoints()
    chatImport:SetPoint("CENTER", LDT.main_frame,"CENTER",0,50)
...
function LDT:MakePresetImportFrame(frame)
	...
	importButton:SetWidth(100)
	importButton:SetCallback("OnClick", function()
		local newPreset = LDT:StringToTable(importString, true)
		if LDT:ValidateImportPreset(newPreset) then
			LDT.main_frame.presetImportFrame:Hide()
			LDT:ImportPreset(newPreset)
...
function LDT:MakePresetCreationFrame(frame)
	...
	frame.PresetCreationEditbox:SetCallback("OnEnterPressed", function(widget, event, text)
		--check if name is valid, block button if so, unblock if valid
		if LDT:SanitizePresetName(text) then
...
function LDT:ValidateImportPreset(preset)
...
function LDT:ImportPreset(preset, fromLiveSession)
...
---Stores r g b values for coloring pulls with LDT:ColorPull()
...
function LDT:SetPresetColorPaletteInfo()
    local preset = LDT:GetCurrentPreset()
    ...
    --LDT:GetCurrentPreset().colorPaletteInfo = db.colorPaletteInfo
end

---GetPresetColorPaletteInfo
function LDT:GetPresetColorPaletteInfo(preset)
...
function LDT:ColorPull(colorValues, pullIdx, preset, bypass, exportColorBlind) -- bypass can be passed as true to color even when automatic coloring is toggled off
    local colorPaletteInfo = LDT:GetPresetColorPaletteInfo(preset)
    local pullIdx = pullIdx or LDT:GetCurrentPull()
    if(pullIdx) then
        local colorValues
        local numberColors
        local r,g,b
...
function LDT:ColorAllPulls(colorValues, startFrom, bypass, exportColorBlind)
    local preset = self:GetCurrentPreset()
    local startFrom = startFrom or 0
    for pullIdx,_ in pairs(preset.value.pulls) do
        if pullIdx >= startFrom then
            LDT:ColorPull(colorValues, pullIdx, preset, bypass, exportColorBlind)
        end
    end
end
...
function LDT:MakeCustomColorFrame(frame)
    ...
    frame.CustomColorFrame.ColorSlider:SetCallback("OnMouseUp", function(event, callbackName, value)
        ...
        LDT:MakeCustomColorFrame(frame)
        LDT:OpenCustomColorsDialog()
    end)
...
function LDT:MakeAutomaticColorsFrame(frame)
...
function LDT:MakePullSelectionButtons(frame)
...
function LDT:PresetsAddPull(index, data, preset)
    preset = preset or self:GetCurrentPreset()
...
---
---@author Dradux
function LDT:PresetsMergePulls(pulls, destination)
...
function LDT:PresetsDeletePull(p, preset)
    preset = preset or self:GetCurrentPreset()
...
function LDT:GetPulls(preset)
    preset = preset or self:GetCurrentPreset()
    return preset.value.pulls
end

function LDT:GetPullsNum(preset)
    preset = preset or self:GetCurrentPreset()
    return table.getn(preset.value.pulls)
end

function LDT:CopyObject(obj, seen)
...
function LDT:PresetsSwapPulls(p1, p2)
...
function LDT:SetMapSublevel(pull)
	--set map sublevel
	...
	for enemyIdx,clones in pairs(db.presets[db.currentDungeonIdx][db.currentPreset[db.currentDungeonIdx]].value.pulls[pull]) do
        if tonumber(enemyIdx) then
            for idx,cloneIdx in pairs(clones) do
                if LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx] then
                    lastSubLevel = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx].sublevel
                end
            end
        end
	end
	if lastSubLevel then
        ...
        if shouldResetZoom then
            LDT:UpdateMap(true,true,true)
        end
	end

	LDT:UpdateDungeonDropDown()
    if shouldResetZoom then LDT:ZoomMapToDefault() end
end

function LDT:SetSelectionToPull(pull)
	--if pull is not specified set pull to last pull in preset (for adding new pulls)
	if not pull then
		local count = 0
		for k,v in pairs(LDT:GetCurrentPreset().value.pulls) do
			count = count + 1
		end
		pull = count
	end
    ...
    if type(pull) == "number" and pull > 0 then
        LDT:GetCurrentPreset().value.currentPull = pull
        LDT:GetCurrentPreset().value.selection = { pull }
        LDT:PickPullButton(pull)

        LDT:DungeonEnemies_UpdateSelected(pull)
    elseif type(pull) == "table" then
        LDT:GetCurrentPreset().value.currentPull = pull[#pull]
        LDT:GetCurrentPreset().value.selection = pull

        LDT:ClearPullButtonPicks()
        for _, pullIdx in ipairs(LDT:GetSelection()) do
            LDT:PickPullButton(pullIdx, true)
            LDT:DungeonEnemies_UpdateSelected(pullIdx)
        end
    end
end
...
function LDT:UpdatePullButtonNPCData(idx)
    if db.devMode then return end
	local preset = LDT:GetCurrentPreset()
	local frame = LDT.main_frame.sidePanel
	local enemyTable = {}
	if preset.value.pulls[idx] then
		...
		for enemyIdx,clones in pairs(preset.value.pulls[idx]) do
            if tonumber(enemyIdx) then
                ...
                if LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx] then
                    local incremented = false
                    local npcId = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["id"]
                    local name = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["name"]
                    local creatureType = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["creatureType"]
                    local level = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["level"]
                    local baseHealth = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["health"]
                    for k,cloneIdx in pairs(clones) do
                        ...
                        if LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["clones"][cloneIdx] then
                            if self:IsCloneIncluded(enemyIdx,cloneIdx) then
                                ...
                                enemyTable[enemyTableIdx].count = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["count"]
                                enemyTable[enemyTableIdx].teemingCount = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["teemingCount"]
                                enemyTable[enemyTableIdx].displayId = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["displayId"]
                                ...
                                enemyTable[enemyTableIdx].ignoreFortified = LDT.dungeonEnemies[db.currentDungeonIdx][enemyIdx]["ignoreFortified"]
                            end
                        end
                    end
                end
    ...
    local pullForces = LDT:CountForces(idx,false)
    local totalForcesMax = LDT:IsCurrentPresetTeeming() and LDT.dungeonTotalCount[db.currentDungeonIdx].teeming or LDT.dungeonTotalCount[db.currentDungeonIdx].normal
    ...
    if idx == 1 then
        ...
        oldPullForces =  LDT:CountForces(idx-1,false)
    end
...
function LDT:ReloadPullButtons()
	local frame = LDT.main_frame.sidePanel
    ...
    for k,pull in ipairs(preset.value.pulls) do
        idx = idx+1
        frame.newPullButtons[idx] = AceGUI:Create("LDTPullButton")
        frame.newPullButtons[idx]:SetMaxPulls(maxPulls)
        frame.newPullButtons[idx]:SetIndex(idx)
        LDT:UpdatePullButtonNPCData(idx)
	...
	frame.newPullButton = AceGUI:Create("LDTNewPullButton")
...
function LDT:ClearPullButtonPicks()
	local frame = LDT.main_frame.sidePanel
...
function LDT:PickPullButton(idx, keepPicked)
    if db.devMode then return end
...
function LDT:AddPull(index)
	LDT:PresetsAddPull(index)
	LDT:ReloadPullButtons()
	LDT:SetSelectionToPull(index)
    LDT:ColorPull()
    LDT:DrawAllHulls()
end

function LDT:SetAutomaticColor(index)
...
function LDT:UpdateAutomaticColors(index)
	if not db.colorPaletteInfo.autoColoring then return end
...
function LDT:ClearPull(index)
    ...
    LDT:ColorPull()
    LDT:DrawAllHulls()
	--LDT:SetAutomaticColor(index)
end

---Moves the selected pull up
function LDT:MovePullUp(index)
	LDT:PresetsSwapPulls(index,index-1)
	LDT:ReloadPullButtons()
	LDT:SetSelectionToPull(index-1)
    LDT:ColorAllPulls(_, index-1)
    LDT:DrawAllHulls()
	--LDT:UpdateAutomaticColors(index - 1)
end

---Moves the selected pull down
function LDT:MovePullDown(index)
	LDT:PresetsSwapPulls(index,index+1)
	LDT:ReloadPullButtons()
	LDT:SetSelectionToPull(index+1)
    LDT:ColorAllPulls(_, index)
    LDT:DrawAllHulls()
	--LDT:UpdateAutomaticColors(index)
end

---Deletes the selected pull and makes sure that a pull will be selected afterwards
function LDT:DeletePull(index)
...
---RenamePreset
function LDT:RenamePreset(renameText)
...
function LDT:GetFirstNotSelectedPullButton(start, direction)
    ...
    local pullIdx = start
    while LDT.U.contains(LDT:GetCurrentPreset().value.selection, pullIdx)
            and LDT.U.isInRange(pullIdx, 1, #LDT:GetCurrentPreset().value.pulls) do
       pullIdx = pullIdx + direction
    end

    if not LDT.U.isInRange(pullIdx, 1, #LDT:GetCurrentPreset().value.pulls) then
        return
    end
...
function LDT:MakeRenameFrame(frame)
	...
	frame.RenameFrame.RenameButton:SetCallback("OnClick",function() LDT:RenamePreset(renameText) end)
...
function LDT:MakeExportFrame(frame)
...
function LDT:MakeDeleteConfirmationFrame(frame)
	...
	frame.DeleteConfirmationFrame.OkayButton:SetCallback("OnClick",function()
		LDT:DeletePreset(db.currentPreset[db.currentDungeonIdx])
...
function LDT:MakeClearConfirmationFrame(frame)
	...
	frame.ClearConfirmationFrame.OkayButton:SetCallback("OnClick",function()
        ...
        if self.liveSessionActive and self:GetCurrentPreset().uid == self.livePresetUID then LDT:LiveSession_SendCommand("clear") end
...
function LDT:OpenConfirmationFrame(width, height, title, buttonText, prompt, callback, buttonText2, callback2)
    local f = LDT.main_frame.ConfirmationFrame
    if not f then
        LDT.main_frame.ConfirmationFrame = AceGUI:Create("Frame")
        f = LDT.main_frame.ConfirmationFrame
...
function LDT:CreateTutorialButton(parent)
    local scale = self:GetScale()
    local sidePanelHeight = LDT.main_frame.sidePanel.PullButtonScrollGroup.frame:GetHeight()
    local helpPlate = {
        FramePos = { x = 0,	y = 0 },
    ...
    if not parent.HelpButton then
        parent.HelpButton = CreateFrame("Button","LDTMainHelpPlateButton",parent,"MainHelpPlateButton")
    ...
    local function TutorialButtonOnClick(self)
        if not HelpPlate_IsShowing(helpPlate) then
            HelpPlate_Show(helpPlate, LDT.main_frame, self)
        else
            HelpPlate_Hide(true)
        end
    end
...
function LDT:RegisterOptions()
    LDT.blizzardOptionsMenuTable = {
        ...
        args = {
            ...
            enable = {
                ...
                set = function(_, newValue)
                    ...
                    if not db.minimap.hide then
                        icon:Show("legendarydungeontools")
                    else
                        icon:Hide("legendarydungeontools")
                    end
                end,
                order = 1,
                width = "full",
	...
	LibStub("AceConfigRegistry-3.0"):RegisterOptionsTable("legendarydungeontools", LDT.blizzardOptionsMenuTable)
	self.blizzardOptionsMenu = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("legendarydungeontools", "legendarydungeontools")
end

function LDT:Round(number, decimals)
	return (("%%.%df"):format(decimals)):format(number)
end

function LDT:RGBToHex(r, g, b)
	r = r*255
	g = g*255
	b = b*255
...
function LDT:DeepCopy(orig)
    local orig_type = type(orig)
    local copy
    if orig_type == 'table' then
        copy = {}
        for orig_key, orig_value in next, orig, nil do
            copy[LDT:DeepCopy(orig_key)] = LDT:DeepCopy(orig_value)
        end
        setmetatable(copy, LDT:DeepCopy(getmetatable(orig)))
...
function LDT:StorePresetObject(obj, ignoreScale, preset)
    --adjust scale
...
function LDT:UpdatePresetObjectOffsets(idx, x, y, preset, silent)
...
function LDT:DrawAllPresetObjects()
...
function LDT:DrawPresetObject(obj, objectIndex, scale, currentPreset, currentSublevel)
...
function LDT:DeletePresetObjects(preset, silent)
	preset = preset or self:GetCurrentPreset()
    if preset == self:GetCurrentPreset() then silent = false end
    local currentSublevel = self:GetCurrentSubLevel()
    for objectIndex,obj in pairs(preset.objects) do
...
function LDT:PresetObjectStepBack(preset, silent)
    preset = preset or self:GetCurrentPreset()
    if preset == self:GetCurrentPreset() then silent = false end
    preset.objects = preset.objects or {}
    local length = 0
...
function LDT:PresetObjectStepForward(preset, silent)
    preset = preset or LDT:GetCurrentPreset()
    if preset == self:GetCurrentPreset() then silent = false end
    preset.objects = preset.objects or {}
    local length = 0
...
function LDT:FixAceGUIShowHide(widget, frame, isFrame, hideOnly)
    frame = frame or LDT.main_frame
    local originalShow,originalHide = frame.Show,frame.Hide
    if not isFrame then
        widget = widget.frame
    end
...
function LDT:GetCurrentAffixWeek()
...
function LDT:PrintCurrentAffixes()
    --run this once so blizz stuff is loaded
    LDT:GetCurrentAffixWeek()
    --https://www.wowhead.com/affixes
    local affixNames = {
        [1] =L["Overflowing"],
        [2] =L["Skittish"],
...
function LDT:IsPlayerInGroup()
    local inGroup = (UnitInRaid("player") and "RAID") or (IsInGroup() and "PARTY")
    return inGroup
end

function LDT:ResetMainFramePos(soft)
...
function LDT:DropIndicator()
    local indicator = LDT.main_frame.drop_indicator
    if not indicator then
        indicator = CreateFrame("Frame", "LDT_DropIndicator")
        ...
        LDT.main_frame.drop_indicator = indicator
...
function LDT:IsShown_DropIndicator()
    local indicator = LDT:DropIndicator()
    return indicator:IsShown()
end

function LDT:Show_DropIndicator(target, pos)
    local indicator = LDT:DropIndicator()
...
function LDT:Hide_DropIndicator()
    local indicator = LDT:DropIndicator()
    indicator:Hide()
end

function LDT:GetSelection()
    if not LDT:GetCurrentPreset().value.selection or #LDT:GetCurrentPreset().value.selection == 0 then
        LDT:GetCurrentPreset().value.selection = { LDT:GetCurrentPreset().value.currentPull }
    end

    return LDT:GetCurrentPreset().value.selection
end

function LDT:GetScrollingAmount(scrollFrame, pixelPerSecond)
    local viewheight = scrollFrame.frame.obj.content:GetHeight()
    return (pixelPerSecond / viewheight) * 1000
end

function LDT:ScrollToPull(pullIdx)
    -- Get scroll frame
    ...
    local pulls = #LDT:GetCurrentPreset().value.pulls + 1 or 1
...
function LDT:CopyPullOptions(sourceIdx, destinationIdx)
    local preset = LDT:GetCurrentPreset()
...
function LDT:GetPullButton(pullIdx)
    local frame = LDT.main_frame.sidePanel
    return frame.newPullButtons[pullIdx]
end

function LDT:UpdatePullButtonColor(pullIdx, r, g, b)
...
--/run LDT:ResetDataCache();
function LDT:ResetDataCache()
    db.dungeonEnemies = nil
    db.mapPOIs = nil
    ReloadUI()
end

function LDT:HardReset()
    legendarydungeontoolsDB = nil
    ReloadUI()
end

function initFrames()
    local main_frame = CreateFrame("frame", "LDTFrame", UIParent)
    tinsert(UISpecialFrames,"LDTFrame")

    --cache dungeon data to not lose data during reloads
    if db.devMode then
        if db.dungeonEnemies then
            LDT.dungeonEnemies = db.dungeonEnemies
        else
            db.dungeonEnemies = LDT.dungeonEnemies
        end
        if db.mapPOIs then
            LDT.mapPOIs = db.mapPOIs
        else
            db.mapPOIs = LDT.mapPOIs
        end
    end

    --merge imported dungeon data
    for dungeonIdx,dungeon in pairs(db.dungeonImport) do
        LDT.dungeonEnemies[dungeonIdx] = dungeon
    end
	...
	main_frame.background:SetColorTexture(unpack(LDT.BackdropColor))
    ...
    local _,_,fullscreenScale = LDT:GetFullScreenSizes()
    main_frame:SetMaxResize(sizex*fullscreenScale,sizey*fullscreenScale)
	LDT.main_frame = main_frame
    ...
    main_frame.mainFrametex:SetColorTexture(unpack(LDT.BackdropColor))
    ...
    main_frame.contextDropdown = L_Create_UIDropDownMenu("LDTContextDropDown", nil)

    LDT:CheckCurrentZone(true)
    LDT:EnsureDBTables()
	LDT:MakeTopBottomTextures(main_frame)
	LDT:MakeMapTexture(main_frame)
	LDT:MakeSidePanel(main_frame)
    LDT:CreateMenu()
	LDT:MakePresetCreationFrame(main_frame)
	LDT:MakePresetImportFrame(main_frame)
    LDT:DungeonEnemies_CreateFramePools()
	--LDT:UpdateDungeonEnemies(main_frame)
	LDT:CreateDungeonSelectDropdown(main_frame)
	LDT:MakePullSelectionButtons(main_frame.sidePanel)
	LDT:MakeExportFrame(main_frame)
	LDT:MakeRenameFrame(main_frame)
	LDT:MakeDeleteConfirmationFrame(main_frame)
	LDT:MakeClearConfirmationFrame(main_frame)
	LDT:CreateTutorialButton(main_frame)
    LDT:POI_CreateFramePools()
    LDT:MakeChatPresetImportFrame(main_frame)
	LDT:MakeSendingStatusBar(main_frame)
	LDT:MakeAutomaticColorsFrame(main_frame)
    LDT:MakeCustomColorFrame(main_frame.automaticColorsFrame)
    LDT:CreateDataImportDialog(main_frame)
    LDT:CreateNoDungeonDataWarning(main_frame)

    --devMode
    if db.devMode and LDT.CreateDevPanel then
        LDT:CreateDevPanel(LDT.main_frame)
    end
    ...
    --tooltip new
    do
        LDT.tooltip = CreateFrame("Frame", "LDTModelTooltip", UIParent, "TooltipBorderedFrameTemplate")
        local tooltip = LDT.tooltip
        ...
        tooltip.String = tooltip:CreateFontString("LDTToolTipString")
	...
	--pullTooltip
	do
		LDT.pullTooltip = CreateFrame("Frame", "LDTPullTooltip", UIParent, "TooltipBorderedFrameTemplate")
        --LDT.pullTooltip:SetOwner(UIParent, "ANCHOR_NONE")
        local pullTT = LDT.pullTooltip
        LDT.pullTooltip:SetClampedToScreen(true)
		LDT.pullTooltip:SetFrameStrata("TOOLTIP")
        LDT.pullTooltip.myHeight = 160
		LDT.pullTooltip:SetSize(250, LDT.pullTooltip.myHeight)
        LDT.pullTooltip.Model = CreateFrame("PlayerModel", nil, LDT.pullTooltip)
        LDT.pullTooltip.Model:SetFrameLevel(1)
        LDT.pullTooltip.Model.fac = 0
        if true then
            LDT.pullTooltip.Model:SetScript("OnUpdate",function (self, elapsed)
                self.fac = self.fac + 0.5
                if self.fac >= 360 then
                    self.fac = 0
                end
            ...
            LDT.pullTooltip.Model:SetPortraitZoom(1)
            LDT.pullTooltip.Model:SetFacing(PI*2 / 360 * 2)
        end

        LDT.pullTooltip.Model:SetSize(110,110)
        LDT.pullTooltip.Model:SetPoint("TOPLEFT", LDT.pullTooltip, "TOPLEFT",7,-7)

        LDT.pullTooltip.topString = LDT.pullTooltip:CreateFontString("LDTToolTipString")
        LDT.pullTooltip.topString:SetFontObject("GameFontNormalSmall")
        LDT.pullTooltip.topString:SetFont(LDT.pullTooltip.topString:GetFont(),10)
        LDT.pullTooltip.topString:SetTextColor(1, 1, 1, 1)
        LDT.pullTooltip.topString:SetJustifyH("LEFT")
        LDT.pullTooltip.topString:SetJustifyV("TOP")
        LDT.pullTooltip.topString:SetHeight(110)
        LDT.pullTooltip.topString:SetWidth(130)
        LDT.pullTooltip.topString:SetPoint("TOPLEFT", LDT.pullTooltip, "TOPLEFT", 110, -7)
        LDT.pullTooltip.topString:Hide()

        local heading = LDT.pullTooltip:CreateTexture(nil, "TOOLTIP")
        heading:SetHeight(8)
        heading:SetPoint("LEFT", 12, -30)
        heading:SetPoint("RIGHT", LDT.pullTooltip, "RIGHT", -12, -30)
        heading:SetTexture("Interface\\Tooltips\\UI-Tooltip-Border")
        heading:SetTexCoord(0.81, 0.94, 0.5, 1)
        heading:Show()

        LDT.pullTooltip.botString = LDT.pullTooltip:CreateFontString("LDTToolTipString")
        local botString = LDT.pullTooltip.botString
        botString:SetFontObject("GameFontNormalSmall")
        botString:SetFont(LDT.pullTooltip.topString:GetFont(),10)
	...
	LDT:initToolbar(main_frame)
    if db.toolbarExpanded then
        main_frame.toolbar.toggleButton:Click()
    end

    --ping
    LDT.ping = CreateFrame("PlayerModel", nil, LDT.main_frame.mapPanelFrame)
    local ping = LDT.ping
    ...
    main_frame.sidePanel.affixDropdown:SetAffixWeek(LDT:GetCurrentPreset().week or (LDT:GetCurrentAffixWeek() or 1))
    LDT:UpdateToDungeon(db.currentDungeonIdx)
	main_frame:Hide()

    --Maximize if needed
    if db.maximized then LDT:Maximize() end
